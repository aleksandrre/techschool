...
function multiplierToExponent(multiplier) {
  // replacing Math.log10(multiplier)
  switch (multiplier) {
    case 0.001:
      return -3;
    case 0.01:
...
function volumeFormatUnit(volume, multiplier) {
  ...
  var paddedStr = pad(
    volScaled.toFixed(0),
    decimalPlaces > 0 ? 8 + decimalPlaces : 8
  );
  if (decimalPlaces < 0) {
    paddedStr =
      paddedStr.substring(0, 8 + decimalPlaces) +
      "." +
      paddedStr.substring(8 + decimalPlaces);
  }
  return paddedStr;
}
...
function usageParser(dataView, result, err) {
  ...
  result.meter_actuality_duration__minutes =
    actualityDurationToMinutesFormat(actuality);
  result.meter_actuality_duration_formatted =
    actualityDurationToFormatStr(actuality);

  result["meter_accumulated_volume__" + result.meter_unit] = volumeFormatUnit(
    result.meter_multiplier * dataView.getUint32(),
    result.meter_multiplier
  );
  ...
  if (deviceStatusSent) {
    result.battery_remaining__years = parseFloat(
      (dataView.getUint8() / 12.0).toFixed(1)
    );
    result.battery_voltage__V = parseFloat(
      (dataView.getUint8() / 100.0 + 1.5).toFixed(2)
    );

    result.internal_temperature__C = dataView.getInt8();
    var bits6 = dataView.getUint8Bits();
    result.internal_temperature_min__C =
      bits6.getBits(4) * -2 + result.internal_temperature__C;
    result.internal_temperature_max__C =
      bits6.getBits(4) * 2 + result.internal_temperature__C;
...
function generalConfigurationParser(dataView, result, err) {
  ...
  if (configuredParameters.radio_lorawan_profile_sent) {
    result.radio_lorawan_profile = lorawanProfileFormat(
      dataView.getUint8(),
      err
    );
  }
  ...
  if (configuredParameters.meter_volume_sent) {
    result["meter_accumulated_volume" + unitPost] =
      dataView.getUint64() / 1000.0;
  }
  if (configuredParameters.meter_volume_offset_sent) {
    result["meter_accumulated_volume_offset" + unitPost] =
      dataView.getInt32() / 1000.0;
  }
  ...
  if (configuredParameters.alert_backflow_sent) {
    ...
    result["alert_backflow_threshold"] =
      valBkThr === 0 ? "disabled" : valBkThr / 1000.0;
  }
  if (configuredParameters.alert_broken_pipe_sent) {
    var valBrThr = dataView.getUint32();
    result["alert_broken_pipe_threshold" + unitPerHour] =
      valBrThr === 0 ? "disabled" : valBrThr / 1000.0;
  }
  ...
  if (configuredParameters.alert_temperature_sent) {
    ...
    result.alert_temperature_threshold_high__C =
      high === -128 ? "disabled" : high;
  }
}
...
function commandParser(dataView, packet_type, result, err) {
  if (packet_type === 0x03) {
    ...
    result.device_local_time_formatted = date
      .toISOString()
      .replace(/.\d+Z$/g, "Z");
...
function sysMessagesParser(dataView, packet_type, result, err) {
  if (packet_type === 0x13) {
    ...
    result.packet_error_reason = packetErrorReasonFormatter(
      dataView.getUint8()
    );
    ...
    result.device_firmware_version =
      dataView.getUint8().toString() +
      "." +
      dataView.getUint8() +
      "." +
      dataView.getUint8();
    ...
    result.hardware_configuration = hardwareConfigFormat(
      dataView.getUint8(),
      err
    );
    dataView.getUint8();
    dataView.getUint8();
    result.device_uptime_accumulated__days = parseFloat(
      (dataView.getUint24() / 24.0).toFixed(2)
    );
